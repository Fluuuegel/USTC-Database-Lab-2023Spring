<template>
    <el-row justify="space-around">
      ...
      <el-col :span="6">
        <el-form
          ...
          size="small"
        >
          <el-form-item v-for="field in client_attr" :key="field.name" :label="field.label">
            <el-input v-model="field.search_ref"/>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" size=large @click="searchClient()">Search</el-button>
          </el-form-item>
        </el-form>
      </el-col>
  ...
  client_attr.forEach(field => field['search_ref'] = '')
  client_attr.forEach(field => field['input_ref'] = '')
  ...
  export default {
    ...
    methods: {
      ...
      fillInput: function(client) {
        this.client_attr.forEach(field => field.input_ref = client[field.name])
      },
  
      addClient: function () {
        let data = {}
        client_attr.forEach(field => data[field.name] = field.input_ref)
      ...
      updateClient: function () {
        let data = {}
        client_attr.forEach(field => data[field.name] = field.input_ref)
        axios.put(client_url + client_attr[0].input_ref + '/', data, auth_config)
          .then(response => {
      ...
      deleteClient: function () {
        axios.delete(client_url + client_attr[0].input_ref + '/', auth_config)
          .then(response => {
